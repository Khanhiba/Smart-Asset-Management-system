...
router.get('/inventory', asyncRoute(async (req, res) => {
  const query = {};
  if (typeof req.query.status === 'string' && ['available', 'assigned', 'maintenance', 'retired', 'lost'].includes(req.query.status)) query.status = req.query.status;
  if (typeof req.query.category === 'string' && req.query.category.trim()) query.category = req.query.category.trim().slice(0, 60);
  if (typeof req.query.location === 'string' && req.query.location.trim()) query.location = req.query.location.trim().slice(0, 100);
