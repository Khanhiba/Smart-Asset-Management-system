...
export const authenticate = asyncRoute(async (req, res, next) => {
  ...
  let payload;
  try { payload = jwt.verify(token, runtime.jwtSecret, { issuer: runtime.jwtIssuer, audience: runtime.jwtAudience }); } catch { throw new AppError('Your session is invalid or expired.', 401); }
